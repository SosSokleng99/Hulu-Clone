...
const login: NextPage = ({logout}: any) => {

    const [email, setEmail] = React.useState("")
    const [password, setPassword] = React.useState("")

    const [message, setMessage] = React.useState<{message: string, type: string}>({message: "", type: ""})
            ...
            <nav className={styles.nav}>
             ...
             {/* <div className={styles.form_message_container}>
                ...
                <div className={styles.form_container}>
                    <h3>{ forgotPassword ?  'Reset Password': 'Log in' }</h3>
                {message && message.message && <div className={styles.form_message} style={{color: "red"}}>{message.message}</div>}


                {forgotPassword ? 
                    <form className={styles.form_wrapper} onSubmit={handleForgotPassword}>
                        <div className={styles.form_input}>
                            <label><b>Email</b></label>
                            <input type="text" placeholder="Enter Email" name="uname" required onChange={e => setForgotEmail(e.target.value)} />
                        </div>
                        <button type="submit" className={styles.login_btn} >Recover Password</button>
                    </form>
                    :
                    <form className={styles.form_wrapper} onSubmit={handleLogIn}>
                        ...
                        <div className={styles.forgot_password_text} onClick= {() => setForgotPassword(!forgotPassword)}>
                              
                              <p>{!forgotPassword ? "Forgot password?" : "Back to Log in"  }    </p>
                        </div>
